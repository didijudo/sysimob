...
class CadastrarEmpreendimentoController extends PerfilAController{
    ...
    public $javascript = array ('jquery', 'jquery.validate', 'cadastro-empreendimento');
    ...
    private function salvar() {
		...
		if (isset($_POST["emp_nome"]) &&
			isset($_POST["emp_descricao"]) &&
			isset($_POST["emp_localizacao"]) &&
			isset($_POST["emp_dtlancamento"]) &&			
			isset($_POST["blo_qtdbloco"]) &&
			isset($_POST["apa_qtdapporandar"]) &&
			isset($_POST["apa_qtdandar"])){
...
// 			var_dump($entEmpreendimento);
			
			$gerEmpreendimento = new GerenciadorEmpreendimento();		
			$linhasInseridasEmp = $gerEmpreendimento->inserir($entEmpreendimento);			
			...
			for ($i = 1; $i <= $qtdBloco; $i++){
				...
				for ($j = 1; $j <= $qtdAndar; $j++) {
					...
					for ($k = 1; $k <= $qtdApPorAndar; $k++) {
						...
						//echo $apa_numero.' - '.$apa_posicao.'<br>';
						$gerApartamento->inserir($entApartamento);
						$apa_numero = $apa_numero - $k; 				
					}						
				}
			}
	...
	public function setTitle() {
		return '..:: AC Engenharia - Cadastrar ::..';
	}
}